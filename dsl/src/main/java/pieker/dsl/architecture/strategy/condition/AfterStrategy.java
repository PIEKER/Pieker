...
import pieker.dsl.architecture.template.condition.After;
import pieker.dsl.util.Util;

import java.util.Arrays;

@Slf4j
public class AfterStrategy implements KeywordStrategy {

    @Override
    public void process(String[] args) {
        log.debug("processing AFTER node...");
    ...
    private void checkArguments(String[] args){
        if (args.length != 2){
            throw new ValidationException("Invalid amount of arguments on AFTER keyword.");
        }
    }
}
